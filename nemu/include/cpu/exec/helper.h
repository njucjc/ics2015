#ifndef __EXEC_HELPER_H__
#define __EXEC_HELPER_H__

#include "cpu/helper.h"
#include "cpu/decode/decode.h"

#define make_helper_v(name) \
	make_helper(concat(name, _v)) { \
		return (ops_decoded.is_data_size_16 ? concat(name, _w) : concat(name, _l)) (eip); \
	}

#define do_execute concat4(do_, instr, _, SUFFIX)

#define make_instr_helper(type) \
	make_helper(concat5(instr, _, type, _, SUFFIX)) { \
		return idex(eip, concat4(decode_, type, _, SUFFIX), do_execute); \
	}

#define make_helper_cc_v(name,cc,type) make_helper_v(concat4(name,cc,_,type))
	
#define make_jcc_helper(cc,condition,type) \
	make_helper(concat6(instr,cc,_,type,_, SUFFIX)) { \
		DATA_TYPE_S disp_temp = instr_fetch(eip+1,DATA_BYTE);\
		int disp = disp_temp;\
		if(condition)\
			cpu.eip += disp;\
		if(DATA_BYTE == 2)\
			cpu.eip &= 0xffff;\
		print_asm(str(instr) str(cc) str(SUFFIX) " %x", eip + disp + DATA_BYTE + 1);\
		return DATA_BYTE+1;\
	}

#define make_setcc_helper(cc,condition,type) \
	make_helper(concat6(instr,cc, _,type, _,SUFFIX)) { \
		int len = concat4(decode_, type, _, SUFFIX) (eip+1);\
		if(condition)\
			OPERAND_W(op_src,1);\
		else\
			OPERAND_W(op_src,0);\
		print_asm(str(instr) str(cc) str(SUFFIX) " %s", op_src->str);\
		return len+1;\
	}

#define make_cmovcc_helper(cc,condition,type)\
	make_helper(concat6(instr,cc, _,type, _,SUFFIX)) {\
		int len = concat4(decode_, type, _, SUFFIX) (eip+1);\
		if(condition)\
			OPERAND_W(op_dest, op_src->val);\
		print_asm(str(instr) str(cc) str(SUFFIX) " %s,%s", op_src->str, op_dest->str);\
		return len+1;\
	}

extern char assembly[];
#ifdef DEBUG
#define print_asm(...) Assert(snprintf(assembly, 80, __VA_ARGS__) < 80, "buffer overflow!")
#else
#define print_asm(...)
#endif

#define print_asm_template1() \
	print_asm(str(instr) str(SUFFIX) " %s", op_src->str)

#define print_asm_template2() \
	print_asm(str(instr) str(SUFFIX) " %s,%s", op_src->str, op_dest->str)

#define print_asm_template3() \
	print_asm(str(instr) str(SUFFIX) " %s,%s,%s", op_src->str, op_src2->str, op_dest->str)

#endif
